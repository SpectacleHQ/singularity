...
<template>
  <section class="relative min-h-[100dvh] flex items-center overflow-hidden">
    ...
    <div class="absolute inset-0 overflow-hidden">
      <div
        class="absolute top-[-20%] left-[-10%] w-[60%] h-[60%] rounded-full bg-gold/8 blur-[120px] animate-[pulse_8s_ease-in-out_infinite]"
      />
      <div
        class="absolute bottom-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full bg-gold-dim/6 blur-[100px] animate-[pulse_12s_ease-in-out_infinite_2s]"
      />
      <div
        class="absolute top-[30%] right-[20%] w-[30%] h-[30%] rounded-full bg-accent/4 blur-[80px] animate-[pulse_10s_ease-in-out_infinite_4s]"
      />
    </div>
    ...
    <div class="relative z-10 max-w-7xl mx-auto px-6 md:px-12 w-full">
      ...
      <div class="mb-8 md:mb-12 anim-fade-up">
        <span
          class="inline-flex items-center gap-2 px-4 py-1.5 border border-gold/20 rounded-full font-mono text-[10px] tracking-[0.2em] uppercase text-gold/80"
        >
      ...
      <div class="relative">
        <div class="anim-fade-up">
          <h1
            class="font-serif text-[clamp(2.2rem,8vw,9rem)] leading-[1.0] tracking-tight text-foreground"
          >
            {{ t('hero.where') }}
          </h1>
        </div>
        ...
        <div class="mt-2 md:mt-4 anim-fade-up">
          <div
            class="font-serif text-[clamp(2.2rem,8vw,9rem)] leading-[1.0] tracking-tight text-foreground/90 flex flex-wrap items-baseline gap-x-0 w-fit"
          >
      ...
      <!-- Bottom info -->
      <div
        class="mt-16 md:mt-24 flex flex-col md:flex-row md:items-end justify-between gap-8 anim-fade-up"
      >
        ...
        <div class="flex items-center gap-6">
          <div class="text-right">
            <p class="font-mono text-[10px] tracking-[0.2em] uppercase text-foreground/30">
              {{ t('hero.basedIn') }}
            </p>
      ...
      <!-- Scroll indicator -->
      <div
        class="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 anim-fade-up"
      >
        <span class="font-mono text-[9px] tracking-[0.3em] uppercase text-foreground/20">{{
          t('hero.scroll')
        }}</span>
        <div class="w-px h-8 bg-linear-to-b from-gold/40 to-transparent animate-pulse" />
      </div>
    </div>
  </section>
</template>
